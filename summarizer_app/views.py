...
def load_and_split_emails(file_path: str):
    """Load emails from uploaded file and split by ---"""
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read().strip()
        emails = [e.strip() for e in content.split("\n---\n") if e.strip()]
        return emails
...
def create_summary_prompt(emails):
    """Prepare the JSON schema and instructions for Gemini AI"""
    json_schema = {
        "type": "array",
        "items": {
            "type": "object",
...
def call_gemini(client, instruction, emails_text, json_schema):
    """Call Gemini AI API with retries"""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            response = client.models.generate_content(
                model="gemini-2.5-flash",
...
def get_writable_media_root():
    """Determine a writable folder for JSON files"""
    if "RENDER" in os.environ:
        # Render's project folder is mostly read-only; use /tmp
...
def home(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        writable_folder = get_writable_media_root()
        
        # Save JSON to writable folder with a unique name
        file_name = f"summary_{uuid.uuid4().hex}.json"
        output_file_path = os.path.join(writable_folder, file_name)
        with open(output_file_path, 'w', encoding='utf-8') as f:
            json.dump(summary_data, f, indent=2)
        
        # Store the full path in session for download
        request.session['json_file'] = output_file_path
    
    return render(request, 'home.html', {'form': form, 'summary_data': summary_data})

def download_file(request):
    file_path = request.session.get('json_file')
